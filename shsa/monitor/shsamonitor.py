...
class SHSAMonitor(Monitor):
    ...
    def __init__(self, model, domain, itoms=None, logfile=None):
        ...
        if logfile is not None:
            self.__logger = Logger(logfile, 'w')
        self.__log_timestamp = -1
        """Current time retrieved from the itoms (key: 't' or 'time') or a
        counter value (0, 1, ..)."""
    ...
    def __log(self, itoms, istatus, out, ostatus):
        ...
        The time is retrieved from the itoms (searched for key 'time' or
        ...
        data = {'itoms': itoms_builtin, 'istatus': istatus_builtin,
                ...
                'substitutions': subs}
        # log
        self.__logger.log(time=timestamp, itoms=itoms_builtin,
                          istatus=istatus_builtin, out=out_builtin,
                          ostatus=ostatus_builtin, substitutions=subs)
